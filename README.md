...
- 安装包：`拾光 Agent Setup 0.2.4.exe`
- 免安装版：`win-unpacked.zip`

`main` 分支更新后会自动刷新 `latest` 预发布包，适合想直接试用最新构建的用户。GitHub Releases 页面里带版本号的正式安装包不会自动替换旧 tag；需要推送新的 `v*` tag，例如 `v0.2.4`，才会生成新的正式 Release。
