...
- 工具具备运行健康记录；仅只读网络请求会在可恢复错误时安全重试一次，写入与执行操作绝不自动重试。
...
桌面端按 `项目 → 工作区 → 会话` 组织任务。每个工作区绑定一个本地目录，会话创建后固定归属该工作区，从历史运行创建的分支也会继承同一工作区。

- 相对路径以当前会话的工作区为基准。
- 写入、补丁、复制、移动和删除被限制在工作区目录内。
- 读取、目录查看、状态查询和搜索可以显式使用操作系统允许访问的绝对路径。
- 终端命令的工作目录固定在工作区并始终需要审批；`cwd` 不是完整的文件系统沙箱，命令中的绝对路径仍可能影响外部文件。
- 从旧版本升级时，原 `workspaceRoot` 会迁移成“默认项目 / 默认工作区”，现有会话会保留并归入该工作区。
