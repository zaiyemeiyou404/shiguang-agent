...
本版重点：普通聊天消息新增安全 Markdown 渲染，支持标题、列表、代码块、行内代码、引用、链接和简单表格。

## 下载使用

在 GitHub Releases 页面下载 Windows 版本：

- 安装包：`拾光 Agent Setup 0.2.23.exe`
- 免安装版：`win-unpacked.zip`

`main` 分支更新后会自动刷新 `latest` 预发布包，适合想直接试用最新构建的用户。GitHub Releases 页面里带版本号的正式安装包不会自动替换旧 tag；需要推送新的 `v*` tag，例如 `v0.2.23`，才会生成新的正式 Release。
